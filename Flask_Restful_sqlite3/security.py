...
def authenticate(username, password):
    user = User.find_by_username(username) # find a user by username the .get method is another way of accessing a dictionary 
...
def identity(payload):  # payload is the content of the JWT token 
    ...
    return User.find_by_id(user_id) # this extracts the specific user from this payload. We can retrieve user id by these 
